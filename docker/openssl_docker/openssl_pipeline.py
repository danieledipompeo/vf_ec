...
def main():
    ...
    handler = logging.FileHandler(os.path.join(configuration.get("paths").get("log_dir", "."), "main.log"))
    logger.addHandler(handler)
    logger.setLevel(logging.DEBUG)
    
    
    project = ProjectFactory.get_project(configuration)
    ...
    try:
        ...
        if not input_csv:
            logger.error("'input_csv' path not found in dataset configuration")
            sys.exit(1)
        
        with open(input_csv, 'r') as f:
            reader = csv.DictReader(f)
            for row in reader:
                if row.get('project') == project.name and 'vuln_commit' in row and 'fix_commit' in row:
